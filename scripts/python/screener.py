...
def load_universe() -> pd.DataFrame:
    ...
    kospi = kospi[[code_col, name_col, marcap_col]].dropna()
    kosdaq = kosdaq[[code_col, name_col, marcap_col]].dropna()
    kospi = kospi.sort_values(marcap_col, ascending=False).head(KOSPI_TOP_N)
    kospi["market"] = "KOSPI"
    kosdaq = kosdaq.sort_values(marcap_col, ascending=False).head(KOSDAQ_TOP_N)
    kosdaq["market"] = "KOSDAQ"

    combined = pd.concat([kospi, kosdaq], ignore_index=True)
    combined.columns = ["ticker", "name", "marcap", "market"]
...
def build_features(
    ticker: str, name: str, market: str, marcap: float, df: pd.DataFrame
...
def main() -> None:
    ...
    for _, row in universe.iterrows():
        ...
        if df is None:
            continue
        f = build_features(
            row["ticker"], row["name"], row["market"], row["marcap"], df
        )
        if f is not None:
            feats.append(f)
