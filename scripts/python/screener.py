...
전략별 특성 (thesis 에 반영):
- `low_buy` (저점 매수): 조정 후 반등 시작. 진입 = 현재가 근처
- `breakout` (박병창식 박스권 돌파): 저항선 돌파 + 거래량 확인. 진입 = 현재가(돌파가)

손절/익절 산정 (중요):
- 각 후보에 `rule_stop_loss`, `rule_take_profit` 가 이미 계산돼 있음 (ATR 변동성 기반, 백테스트 검증 산식: 손절=현재가-2*ATR, 익절=현재가+3*ATR).
- **기본적으로 이 값을 stop_loss / take_profit 에 그대로 사용**하라.
- 구조적 지지/저항(전저점, 박스 상단)이 ATR 손절보다 명백히 가까우면 그쪽으로 조정 가능. 단 변경 시 thesis 에 근거 명시.
- entry_hint 는 현재가(`close`) 근처로.

필드 해석 (반드시 지킬 것):
- `market_cap` 은 이미 한국어 표기로 포맷된 문자열(예: "12,900억원" / "1.29조원"). **숫자를 재계산하지 말고 그대로 인용**.
- `atr_14` 는 변동성(원). 클수록 손절폭 넓음. `turnover_eok_20` 은 20일 평균 거래대금(억원).
...
- `rs_rating` (0~100) — 유니버스 내 12개월 수익률 백분위(상대강도). **70+ 는 강한 종목의 눌림목**(선호), 30↓ 는 약한 하락추세(저점매수라도 "떨어지는 칼날" 위험 → risks 명시·confidence 하향). `return_252d_pct` 는 12개월 수익률 원본.
...
class CandidateFeatures:
    ...
    marcap: float  # 억원
    # 상대강도 (RS) — 12개월 수익률 + 유니버스 내 백분위(0~100)
    # return_252d_pct 는 종목별 독립 계산, rs_rating 은 quant_filter 에서 백분위화
    return_252d_pct: float = 0.0
    ...
    atr_14: float = 0.0  # ATR(14) — 변동성 기반 손절/익절 산정용
    turnover_eok_20: float = 0.0  # 20일 평균 거래대금 (억원) — 유동성
    # ATR 기반 룰로 산정한 손절/익절 (Claude 즉흥 결정 대체)
    rule_stop_loss: float = 0.0
    rule_take_profit: float = 0.0
...
def build_features(
    ...
    # 12개월(252거래일) 수익률 — RS rating 산정용. 데이터 부족 시 가용 최장 기간.
    if len(close) >= 253:
        base_252 = _safe_float(close.iloc[-253]) or 0.0
    elif len(close) > 1:
        base_252 = _safe_float(close.iloc[0]) or 0.0
    else:
        base_252 = 0.0
    ret_252d = _safe_pct(last, base_252) if base_252 > 0 else 0.0
    ...
    # ATR(14) — True Range 14일 평균. 백테스트와 동일 산식.
    atr_14 = 0.0
    if len(df) >= 15:
        ...
        atr_14 = atr_val if atr_val and atr_val > 0 else 0.0

    # 20일 평균 거래대금 (억원) — 유동성
    turnover_eok_20 = 0.0
    if len(close) >= 20:
        to = _safe_float((close * volume).tail(20).mean())
        turnover_eok_20 = (to / 1e8) if to else 0.0

    # ATR 기반 룰 손절/익절 (Claude 즉흥 결정 대체). 백테스트와 동일: 2*ATR / 3*ATR
    rule_stop_loss = round(last - 2.0 * atr_14) if atr_14 > 0 else 0.0
    rule_take_profit = round(last + 3.0 * atr_14) if atr_14 > 0 else 0.0
    ...
    return CandidateFeatures(
        ...
        return_252d_pct=round(ret_252d, 2) if ret_252d is not None else 0.0,
        ...
        turnover_eok_20=round(turnover_eok_20, 1),
        rule_stop_loss=rule_stop_loss,
        rule_take_profit=rule_take_profit,
    )
...
def strategy_low_buy(f: CandidateFeatures, config: dict) -> bool:
    ...
    if c.get("enabled", 1.0) < 0.5:
        return False
    # RS(상대강도) 게이트 — 약한 하락추세 종목의 저점매수("떨어지는 칼날") 회피.
...
# 시장별 정렬 보너스 — KOSDAQ 단기 스윙 성과가 KOSPI 대비 우월.
# (2026-06 백테스트 run #7: KOSDAQ 70건 +3.63% vs KOSPI 146건 +0.81%)
# 게이트가 아니라 정렬 점수 보너스라 KOSPI 도 통과 가능 — Top-N 슬롯 경쟁에서
# KOSDAQ 이 우선되어 KOSPI 비중이 자연스럽게 감소. 표본 보존.
KOSDAQ_SORT_BONUS = 0.05
...
def quant_filter(
    ...
    RS rating: 게이트 진입 전 유니버스(feats 전체) 12개월 수익률 백분위로 채움.
    이후 정렬 가중치 + low_buy RS 게이트에 사용.
    """
    _assign_rs_rating(feats)
    ...
    # 정렬: 전략 우선순위 → (RS 0.4 + 거래량비 0.3 + 저점 근접 0.3).
    # RS 비중을 둬서 "강한 종목의 눌림목"을 상위로(= Claude 우선 전달).
    out.sort(
        key=lambda x: (
            ...
            + (1 - x.pos_52w) * 0.3
            + (KOSDAQ_SORT_BONUS if x.market == "KOSDAQ" else 0.0),
        ),
        reverse=True,
    )
    return out
...
def _assign_rs_rating(feats: list[CandidateFeatures]) -> None:
    """유니버스 내 12개월 수익률 백분위(0~100)를 각 후보의 rs_rating 에 채움.
    데이터 부족(return_252d_pct==0)인 종목은 백분위 산정에서 제외하고 중립(50)."""
    valued = [f for f in feats if f.return_252d_pct != 0.0]
    n = len(valued)
    if n < 2:
        for f in feats:
            f.rs_rating = 50.0
        return
    ranked = sorted(valued, key=lambda x: x.return_252d_pct)
    for i, f in enumerate(ranked):
        f.rs_rating = round(i / (n - 1) * 100, 1)
    for f in feats:
        if f.return_252d_pct == 0.0:
            f.rs_rating = 50.0
...
def save_picks(
    ...
    for i, p in enumerate(picks, start=1):
        ...
        # Claude 가 손절/익절 누락·이상치면 ATR 룰 값으로 폴백 (일관성 보장)
        stop = p.get("stop_loss")
        take = p.get("take_profit")
        entry = p.get("entry_hint")
        if f:
            if not stop or stop <= 0:
                stop = f.rule_stop_loss or None
            if not take or take <= 0:
                take = f.rule_take_profit or None
